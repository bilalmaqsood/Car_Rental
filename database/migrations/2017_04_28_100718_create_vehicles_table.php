...
class CreateVehiclesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('vehicles', function (Blueprint $table) {
            ...
            $table->boolean('pickup')->nullable()->comment('yes / no if pick by owner');
            $table->boolean('delivery')->nullable()->comment('yes / no if delivery by owner');
            $table->string('pickup_location', 100)->nullable()->comment('vehicle pickup location in latitude and longitude');
            $table->string('return_location', 100)->nullable()->comment('vehicle return location in latitude and longitude');
            $table->string('location', 100)->comment('vehicle location in latitude and longitude');
            $table->decimal('delivery_charges')->nullable()->comment('delivery charges per week');
            $table->decimal('rent')->comment('rent per week');
            $table->decimal('insurance')->comment('insurance per week');
            $table->decimal('mile_cap')->nullable()->comment('mileage Cap per week');
            $table->decimal('after_mile')->nullable()->comment('after mileage per mile price');
            $table->decimal('deposit')->nullable()->comment('deposit on vehicle booking');
